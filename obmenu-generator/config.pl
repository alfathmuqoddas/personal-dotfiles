...
our $CONFIG = {
  "editor"              => "leafpad",
  "Linux::DesktopFiles" => {
                             desktop_files_paths     => ["$ENV{HOME}/.local/share/applications", "/usr/share/applications", "/usr/local/share/applications"],
                             gtk_rc_filename         => "$ENV{HOME}/.gtkrc-2.0",
                             icon_dirs_first         => "/usr/share/pixmaps",
                             icon_dirs_last          => [ 
	"/usr/share/pixmaps, /usr/share/icons/hicolor",
                             ...
                             skip_entry => [                                 
{key => 'Exec', re => qr/(menu-openbox|menu-debian|jwm-theme-change|nitrogen)/i},
],
                             skip_filename_re        => undef,
                             skip_svg_icons          => 0,
                             strict_icon_dirs        => undef,
                             substitutions           => [
	{key => 'Exec', re => qr/gparted-pkexec/, value => "gsu gparted"},
	{key => 'Exec', re => qr/synaptic-pkexec/, value => "gsu synaptic"},
],
